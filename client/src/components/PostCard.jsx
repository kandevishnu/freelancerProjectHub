  ...
  const renderContent = () => {
    switch (postType) {
      case 'job':
        return (
          <div className="border border-gray-200 rounded-lg p-4 mt-2">
            ...
            {content.jobBudget && <p className="text-green-600 font-semibold">Budget: ${content.jobBudget}</p>}
            <p className="mt-2 text-gray-700 line-clamp-3">{content.jobDescription}</p>
            {project && (
              <Link
                to={`/project/${project}`}
      ...
      case 'showcase':
        return (
          <div>
            <p className="mb-2 text-gray-700">{content.text}</p>
            <img src={content.imageUrl} alt="Showcase" className="mt-2 rounded-lg w-full object-cover" />
          </div>
        );
      case 'text':
      default:
        return <p className="text-gray-700">{content.text}</p>;
    }
  };
    ...
    <div className="bg-white rounded-lg shadow-md p-5 mb-6">
      <div className="flex items-center mb-4">
        <Link to={`/profile/${author._id}`} className="flex items-center group">
            ...
            <div className="ml-3">
                <p className="font-semibold text-gray-900 group-hover:underline">{author.name}</p>
      ...
      <div className="border-t border-gray-200 pt-2 flex justify-around text-gray-600">
        <button onClick={handleLike} className={`flex items-center gap-2 hover:bg-gray-100 p-2 rounded-lg w-full justify-center transition-colors ${ isLiked ? 'text-blue-600' : 'text-gray-600' }`} >
        ...
        <button onClick={() => setShowComments(!showComments)} className="flex items-center gap-2 hover:bg-gray-100 p-2 rounded-lg w-full justify-center">
      ...
      {showComments && (
        <div className="border-t border-gray-200 mt-3 pt-4">
          <form onSubmit={handleCommentSubmit} className="flex gap-2 mb-4">
            ...
            />
            <button type="submit" className="bg-blue-600 text-white px-4 rounded-full font-semibold">Post</button>
          </form>
          <div className="space-y-3">
            {comments.slice(0, visibleCommentsCount).map((comment) => (
              <div key={comment._id} className="flex items-start gap-2">
                <div className="w-8 h-8 rounded-full bg-gray-200 flex-shrink-0 items-center justify-center flex font-bold text-gray-600">
